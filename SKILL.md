...
本 Skill 采用分层架构设计，确保逻辑清晰且易于扩展：

- **`SKILL.md`**: 声明文件，定义 Skill 名称、描述、触发词及整体架构。
- **`core/` (核心逻辑层)**: 
    - `planner.js`: 任务调度中心，负责拆解视频处理链路。
    - `analyst.js`: 智能分析引擎，对比“用户习惯”与“转录文本”，识别语气词、错误和停顿。
- **`tools/` (工具层)**: 
    - `ffmpeg_wrapper.sh`: 封装 FFmpeg 指令，处理视频转音频等媒体操作。
    - `asr_engine.js`: 对接火山引擎 ASR 接口，实现语音到文本的精准转写。
- **`config/` (配置与知识库)**: 
    - `principles/`: 存放剪辑的核心原则（如：语气词库、重复识别逻辑）。
    - `preferences/`: 存放用户的个性化偏好（如：停顿阈值、输出格式）。
    - `vendor.env`: 管理 API Keys 及第三方服务配置。
- **`templates/` (输出模板)**: 
    - `review.html`: 可视化的剪辑建议报告，方便用户预览修改点。
- **`tests/` (测试用例)**: 
    - `mock_subtitles.json`: 模拟转录数据，用于核心分析逻辑的单元测试。
